...
def eval_detection_voc(
    ...
    prec, rec = calc_detection_voc_prec_rec(pred_bboxes, pred_labels, pred_scores, gt_bboxes, gt_labels, gt_difficults,
    ...
    for pred_bbox, pred_label, pred_score, gt_bbox, gt_label, gt_difficult in six.moves.zip(pred_bboxes, pred_labels,
        ...
        for l in np.unique(np.concatenate((pred_label, gt_label)).astype(int)):
            ...
            if len(pred_bbox_l) == 0:
                continue
            if len(gt_bbox_l) == 0:
                match[l].extend((0,) * pred_bbox_l.shape[1])
                continue
